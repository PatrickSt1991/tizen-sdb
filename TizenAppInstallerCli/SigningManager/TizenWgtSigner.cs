...
namespace TizenSdb.SigningManager
{
    public static class TizenWgtSigner
    {
            ...
            string? backupPath = null)
        {
            ...
            // Load leafs (ephemeral so no machine key container needed)
            var authorLeaf = LoadLeaf(authorPfxPath, password);
            var distributorLeaf = LoadLeaf(distributorPfxPath, password);

            // Load full collections from PFX (leaf + intermediates present in the PFX)
            var authorColl = new X509Certificate2Collection();
            authorColl.Import(authorPfxPath, password, X509KeyStorageFlags.Exportable);

            var distributorColl = new X509Certificate2Collection();
            distributorColl.Import(distributorPfxPath, password, X509KeyStorageFlags.Exportable);
        ...
        private static X509Certificate2 LoadLeaf(string pfxPath, string password)
        {
            ...
            {
                // Fallback for environments without EphemeralKeySet support
                return new X509Certificate2(
                    pfxPath, password,
